...
class Recommend:
    """
    Class provides methods to generate movie recommendations given a dataset
    
    The dataset must be formatted with columns of:
    ...
    def __init__(self, file_dataset, file_ground_truth):
        """initialize instance of Recommend

        Args:
            file_dataset (str): path, file name, and extension of the properly formatted dataset
            file_ground_truth (str): path, file name, and extension of the properly formatted ground truth data
        """
        self.file_dataset = file_dataset
        self.file_ground_truth = file_ground_truth

    # CREATING SIMULARITY MATRIX
    def create_matrix(self, file_sim_matrix_json):
        """ Generates json file with movie recommendations

        Args:
            file_sim_matrix_json (str): full path to the destination directory and file name

        Returns:
            json file: 20 most relevant movie recommendations for each movie in a dataset
        """

    # open the movie database csv file
        try:
            df = pd.read_csv(self.file_dataset)
    ...
    # this function changes the movie id to the movie index in the dataset
        ...
        try:
            with open(self.file_ground_truth, 'r') as f:
    ...
    # processing the overview text to clean the text and remove stop words
        def remove_stop_words(overview):
            ...
            filtered_overview = [word for word in token_overview if word not in self.stop_words]
    ...
    def recommend_movie(self, title, number_of_movies, path_sim_matrix_json):
        """Uses json file with similar movies to return recommended movies

        Args:
            title (str): title of movie to search recommendations for
            number_of_movies (int): number of movies to be recommeded, requires less than 20 (inclusive)

        Returns:
            recommendations: array of recommendations, sorted in non-ascending order
        """

        # load the simularity matrix
        with open(path_sim_matrix_json, 'r') as file:
